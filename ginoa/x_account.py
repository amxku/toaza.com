...
class authset(RequestHandler):
    @tornado.web.authenticated
    def get(self):
        countent = self.render_template('user_settings.html', title=u'设置',)
        self.write(countent)
    ...
    def post(self):
        ...
        if username:
            ...
            if check_username and check_username.uid != gnauid:
                ...
                self.session['gnaname'] = username
                self.session.save()
                self.logaw('setting', u'设置个人信息', '0', '0', '0', '0')
                #记录日志(type,des,aid,cid,nid,puid)
        else:
    ...
    def post(self):
        ...
        if myfile:
            ...
            if not avatar:
                ...
                self.logaw('avatar', u'上传头像', '0', '0', '0', '0')
                #记录日志(type,des,aid,cid,nid,puid)
...
class alerts(RequestHandler):
    ...
    def get(self):
        ...
        title = u'提醒消息'
        countent = self.render_template('user_alerts.html', title=title, lids=lids)
        self.write(countent)
