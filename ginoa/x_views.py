...
class newcomment(RequestHandler):
    ...
    def post(self):
        ...
        aid = int(self.get_argument('aid', '')) #获取主题ID
        if aid:
            ...
            if arts:
                ...
                if comment:
                    ...
                    Article().article_comcount(temptime(),aid) #更新回复数量

                    logid= self.logaw('comments',u'回复评论',aid,str(comid),'0','0') #记录日志(type,des,aid,cid,nid,puid)
                    ##消息提醒################################################################
                    #排除自己
...
class newpost(RequestHandler):
    ...
    def get(self):
        ...
        aid = self.get_argument('id', '') #获取主题ID
        if aid:
            #编辑主题
            nnodeid = ''
    ...
    def post(self):
        ...
        if aid:
            ...
            if (self.session['gnauid'] == articlei.uid and temptime()-articlei.aptime<1800) or self.session['gnaflag']=='675': #判断权限
                if articlei:
                    if not tags:
                    ...
                    self.logaw('editarticle',u'修改主题',aid,'0','0','0') #记录日志(type,des,aid,cid,nid,puid)
                    self.redirect(tourl)
                else:
                    self.get_error('/','10',u'参数异常',SITE_NAME)
                    return
            ...
            if not checkaid:
                ...
                if not atags[0]:
                    ...
                    self.logaw('article',u'新主题',aid,'0','0','0')  #记录日志(type,des,aid,cid,nid,puid)
                    for atg in atags:
                        if atg:
